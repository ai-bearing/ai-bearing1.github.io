  ...
  \begin{description}[font = \normalfont\scshape]
    \item[Base Case] Suppose $n = k+1$. Then, we can find the following:
      \begin{align*}
        ...
        e(G) &> \frac{k(k+1)}{2} - 1
      \end{align*}
      This means $e(G) = \frac{k(k+1)}{2}$ in the base case, so $G$ is the complete graph on $k+1$ vertices, so $\delta(G) = k$, meaning $T\subseteq G$.
    \item[Inductive Hypothesis] If $n > k+1$, $e(G) > n(k-1) - {k\choose 2}$, and $\delta(G) < k$, then we have that $e(G-x) > (n-1)(k-1) - {k\choose 2}$, where $d(x) = \delta(G)$.
    \item[Proof] Let $G' = G-x$, where $\delta(G)\leq k-1$ and $d(x) = \delta(G)$. Then, we have the following:
      \begin{align*}
        e(G') &= e(G) - \delta(G)\\
        e(G') &\geq e(G) - (k-1) \\
      ...
      \end{align*}
      Therefore, the inductive hypothesis is proven, and we have shown the formula to be correct.
  \end{description}
\end{problem}
}\end{document}
